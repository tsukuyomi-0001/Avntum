            ...
            if objects.type == 'VARASSIGN':
                ...
                for token in objects.value:
                    if token[0] == 'LPARAN':
                        ...
                        string += token[1]
                string += ';\n'
            elif objects.type == 'OUTCALL':
                header.add('#include <iostream>\n')
                string = f'cout << {token[1]} << endl;\n'
            lines.append(string)
        lines.append(tail)

        with open(file, 'w') as f:
            f.writelines(list(header) + lines)
